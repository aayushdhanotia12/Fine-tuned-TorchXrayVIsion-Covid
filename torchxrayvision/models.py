...
#         return out
class DenseNet(nn.Module):
    """A modification of the Densenet model for specific use with three classes: 
    COVID pneumonia, non-COVID pneumonia, and non-pneumonia."""

    def __init__(self, num_classes=3, in_channels=1, weights=None):
        super(DenseNet, self).__init__()
        self.weights = weights
    ...
    def load_pretrained_weights(self):
        # Path to the weights file (local or URL)
        weights_path = 'https://github.com/mlmed/torchxrayvision/releases/download/v1/nih-pc-chex-mimic_ch-google-openi-kaggle-densenet121-d121-tw-lr001-rot45-tr15-sc15-seed0-best.pt'

    # Check if the weights need to be downloaded from a URL
        if weights_path.startswith('http://') or weights_path.startswith('https://'):
        # Download logic here
            import requests
            response = requests.get(weights_path)
            with open('local_weights_path.pth', 'wb') as f:
                f.write(response.content)
            weights_path = 'local_weights_path.pth'

    # Load the state dictionary from the file
        state_dict = torch.load(weights_path, map_location='cpu')

    # If you need to load part of the state dict (e.g., all but the classifier)
    # You might need to filter out some keys
