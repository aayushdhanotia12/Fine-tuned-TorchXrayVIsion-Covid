...
#     'Covid'
# ]
default_pathologies = ["COVID pneumonia", "non-COVID pneumonia", "non-pneumonia"]
# Defined specific pathologies
specific_pathologies = ["COVID pneumonia", "non-COVID pneumonia", "non-pneumonia"]
...
#Custom dataset for Veytel
class PneumoniaXRayDataset(Dataset):
    pathologies = ["non-pneumonia", "non-COVID pneumonia", "COVID pneumonia"]
    def __init__(self, csv_file, img_dir, transform=None, data_aug=None):
        """
        Args:
            csv_file (string): Path to the csv file with annotations.
            img_dir (string): Directory with all the images.
            transform (callable, optional): Optional transform to be applied on a sample.
            data_aug (callable, optional): Optional data augmentation to be applied on a sample.
        """
        self.annotations = pd.read_csv(csv_file)
        self.img_dir = img_dir
        self.transform = transform
        self.data_aug = data_aug
        self.labels = np.array(self.annotations['label'].values) # Assuming 'label' is the column in CSV with pathology labels
    def __len__(self):
    ...
    #     label = int(self.annotations.iloc[index, 1] == 'positive covid') * 2 + \
    #             int(self.annotations.iloc[index, 1] == 'positive pneumonia')
    ...
    def __getitem__(self, index):
        ...
        age = self.annotations.iloc[index, 0]
        sex = self.annotations.iloc[index, 1]
        label = self.annotations.iloc[index, 2]
        dataset_ID = self.annotations.iloc[index, 4]
        ...
        # Creating a dictionary for all the extracted information
        sample = {'image': image,
                  'label': label,
                  'age': age,
                  'sex': sex,
